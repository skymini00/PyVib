...
class LV_DLLInterface_BGProcess_Adaptor:
    ...
    def LoadOCTDispersion(self, magWin, phaseCorr):
        ...
        self.collMsgQ.put(msg, timeout=0.5)
        err = self.statusQ.get(timeout=0.5)
        if err is None:
            err = -1
    ...
    def CloseFPGA(self):
        ...
        resp = 0
        if not self.statusQ.empty():
            resp = self.statusQ.get(timeout=0.5)
            if err is None:
                resp = -1
        return resp
    ...
    def GetData(self):
        ...
        if not self.rawDataQ.empty():
            data = self.rawDataQ.get(timeout=0.5)
        return data
        
    def GetStatus(self):
        status = None
        if not self.statusQ.empty():
            status = self.statusQ.get(timeout=0.5)
        return status
